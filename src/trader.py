    ...
    def _price_thread(self):
        while not self._stop_evt.is_set():
            try:
                ...
                log.warning("price_thread: %s", exc)
            time.sleep(5)
    ...
    def _candle_thread(self):
        ...
        while not self._stop_evt.is_set():
            ...
            try:
                ...
                log.warning("candle_thread: %s", exc)
        ...
        while not self._stop_evt.is_set():
            ...
            try:
                ...
                if cur == last_btc and cur > 0:
                    log.warning("Prices stale — force refresh")
                    p = get_prices(cfg.HL_REST_URL)
                    if p:
                        self.state.update_prices(p)
    ...
    def _snapshot_thread(self):
        while not self._stop_evt.is_set():
            ...
            try:
                ...
                log.warning("snapshot_thread: %s", exc)

    # ── Main trading loop ─────────────────────────────────────────────────────
    def _trading_loop(self):
        last_day  = __import__("datetime").datetime.now().day
        last_week = __import__("datetime").datetime.now().isocalendar()[1]
